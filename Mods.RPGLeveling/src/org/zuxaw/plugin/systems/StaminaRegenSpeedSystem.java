...
public class StaminaRegenSpeedSystem
extends EntityTickingSystem<EntityStore> {
    ...
    public StaminaRegenSpeedSystem(@Nonnull StatsService statsService, @Nonnull LevelingConfig config) {
    ...
    public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
        ...
        int allocatedPoints = playerData.getAllocatedPoints("StaminaRegenDelay");
        ...
        if (currentStamina >= (maxStamina = staminaValue.getMax())) {
            this.previousStaminaValues.remove(playerRef.getUuid());
            return;
        }
        ...
        boolean isRegenerating = previousStamina != null && currentStamina > previousStamina.floatValue();
        this.previousStaminaValues.put(playerUuid, Float.valueOf(currentStamina));
        if (!isRegenerating) {
            return;
        }
        float speedMultiplier = Formulas.staminaRegenSpeedMultiplier(allocatedPoints, this.config);
        float naturalIncrease = currentStamina - previousStamina.floatValue();
        float acceleratedIncrease = naturalIncrease * speedMultiplier;
        float newStamina = Math.min(maxStamina, previousStamina.floatValue() + acceleratedIncrease);
